...
@section('title')
<title>Câu chuyện khách hàng</title>
@endsection
...
<div>
    <div class='main-wrapper'>
        ...
        <div class="banner-content">
            <p class="banner-text-small set-font-size" data-font-size="28">TRANG CHỦ. <a style="text-decoration:none; color: white;" href="{{ route('cau-chuyen-khach-hang') }}">CÂU CHUYỆN KHÁCH HÀNG</a></p>
            <p class="banner-text-big set-font-size set-height-block" data-font-size="96" data-height-block="113">Câu chuyện<br>khách hàng</p>
        </div>
        </div>
    </div>
    ...
    <div class="content">
        <div class="header-content col">
            ...
            <div class="col-xl-3 col-lg-3 col-sm-4 col-4 right-content">
                ...
                @foreach($categories as $category)
                    <div class="post-categoris"><span><a style="color: #0062FF;" href="{{ route('cau-chuyen-khach-hang').'?category='.$category->id }}">{{ $category->title }}</a></span></div>
                @endforeach
            </div>
            <div class="col-xl-3 col-lg-3 col-sm-12 col-12" style="padding: 0px;">
                <button href="#" class="button-QA" style="width: 100%;">
        ...
        <div class="body-content col">
                ...
                @foreach($storys as $story)
                    <div class="post-content">
                        <div class="post-content__body">
                            <a href="{{ route('story.detail', $story->id) }}"><img style="width:100%" src="{{ asset('public/storage/images/post/'.$story->id.'.'.$story->extension) }}" alt="Image post" class="image_post"></a>
                            <br>
                            <a href="{{ route('story.detail', $story->id) }}" style="text-decoration:none"><span class="title_post set-font-size" data-font-size="28">{{ $story->title }}</span></a><br>
                            <span class="time_post set-font-size" data-font-size="22">{{ date('M d, Y', strtotime($story->created_at)) }}</span><br>
                            <p class="content_post set-font-size" data-font-size="18">{{ $story->short_content }}</p>
                            <button data-link="{{ route('story.detail', $story->id) }}" class="button-QA post-detail set-padding-button" data-padding-left="31" data-padding-top="25">
                                <span class="set-font-size" data-font-size="22">ĐỌC THÊM</span>
                            </button>
                        </div>
                    </div>
                @endforeach
                <center>{{ $storys->links('pagination.default') }}</center>
            </div>
            ...
            <div class="col-xl-3 col-lg-3 col-sm-4 col-4 right-content sticky-detail">
                ...
                @foreach($categories as $category)
                    <div class="post-categoris set-font-size" data-font-size="18"><span><a style="color: #0062FF;" href="{{ route('cau-chuyen-khach-hang').'?category='.$category->id }}">{{ $category->title }}</a></span></div>
                @endforeach
            </div>
        </div>
    </div>
