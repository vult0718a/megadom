    ...
    <div class="content">
        <div class="header-content col">
            ...
            <div class="col-xl-3 col-lg-3 col-sm-4 col-4 right-content">
                ...
                @foreach($categories as $category)
                    <div class="post-categoris"><span><a style="color: #0062FF;" href="{{ route('thong-tin-suc-khoe').'?category='.$category->id }}">{{ $category->title }}</a></span></div>
                @endforeach
            </div>
            <div class="col-xl-3 col-lg-3 col-sm-12 col-12" style="padding: 0px;">
                <button href="#" class="button-QA" style="width: 100%;">
        ...
        <div class="body-content col">
                ...
                @foreach($healths as $health)
                    <div class="post-content">
                        <div class="post-content__body">
                            <a href="{{ route('health.detail', $health->id) }}"><img style="width:100%" src="{{ asset('public/storage/images/post/'.$health->id.'.'.$health->extension) }}" alt="Image post" class="image_post"></a>
                            <br>
                            <a href="{{ route('health.detail', $health->id) }}" style="text-decoration:none"><span class="title_post set-font-size" data-font-size="28">{{ $health->title }}</span></a><br>
                            <span class="time_post set-font-size" data-font-size="22">{{ date('M d, Y', strtotime($health->created_at)) }}</span><br>
                            <p class="content_post set-font-size" data-font-size="18">{{ $health->short_content }}</p>
                            <button data-link="{{ route('health.detail', $health->id) }}" class="button-QA post-detail set-padding-button" data-padding-left="31" data-padding-top="25">
                                <span class="set-font-size" data-font-size="22">ĐỌC THÊM</span>
                            </button>
                        </div>
                    </div>
                @endforeach
                <center>{{ $healths->links('pagination.default') }}</center>
            </div>
            ...
            <div class="col-xl-3 col-lg-3 col-sm-4 col-4 right-content sticky-detail">
                ...
                @foreach($categories as $category)
                    <div class="post-categoris set-font-size" data-font-size="18"><span><a style="color: #0062FF;" href="{{ route('thong-tin-suc-khoe').'?category='.$category->id }}">{{ $category->title }}</a></span></div>
                @endforeach
            </div>
        </div>
    </div>
