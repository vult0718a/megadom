...
<div>
    <div class='main-wrapper'>
        ...
        <div class="banner-content">
            <p class="banner-text-small set-font-size" data-font-size="28">TRANG CHỦ. <a style="text-decoration:none; color: white;" href="{{ route('post') }}">TIN TỨC</a></p>
            <p class="banner-text-big set-font-size set-height-block" data-font-size="96" data-height-block="113">Tin tức</p>
        </div>
        </div>
    </div>
    ...
    <div class="content">
        <div class="header-content col">
            ...
            <div class="col-xl-3 col-lg-3 col-sm-4 col-4 right-content sticky-detail">
                ...
                @foreach($categories as $category)
                    <div class="post-categoris"><span><a style="color: #0062FF;" href="{{ route('post').'?category='.$category->id }}">{{ $category->title }}</a></span></div>
                @endforeach
            </div>
            <div class="col-xl-3 col-lg-3 col-sm-12 col-12" style="padding: 0px;">
                <button href="#" class="button-QA" style="width: 100%;">
        ...
        <div class="body-content col">
                ...
                @foreach($posts as $post)
                    <div class="post-content">
                        <div class="post-content__body">
                            <a href="{{ route('post.detail', $post->id) }}"><img style="width:100%" src="{{ asset('public/storage/images/post/'.$post->id.'.'.$post->extension) }}" alt="Image post" class="image_post"></a>
                            <br>
                            <a href="{{ route('post.detail', $post->id) }}" style="text-decoration:none"><span class="title_post set-font-size" data-font-size="28">{{ $post->title }}</span></a><br>
                            <span class="time_post set-font-size" data-font-size="22">{{ date('M d, Y', strtotime($post->created_at)) }}</span><br>
                            <p class="content_post set-font-size" data-font-size="18">{{ $post->short_content }}</p>
                            <button data-link="{{ route('post.detail', $post->id) }}" class="button-QA post-detail set-padding-button" data-padding-left="31" data-padding-top="25">
            ...
            <div class="col-xl-3 col-lg-3 col-sm-4 col-4 right-content sticky-detail">
                ...
                @foreach($categories as $category)
                    <div class="post-categoris set-font-size" data-font-size="18"><span><a style="color: #0062FF;" href="{{ route('post').'?category='.$category->id }}">{{ $category->title }}</a></span></div>
                @endforeach
            </div>
        </div>
    </div>
